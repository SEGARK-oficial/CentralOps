...
- **50 regras por ciclo**: máximo de regras inflight carregadas e avaliadas num ciclo de coleta. A carga ordena por `id` crescente e corta no teto — **sem offset, sem cursor e sem rotação**. Não há ondas nem fila: o **mesmo** conjunto roda em todo ciclo e as excedentes **nunca** são avaliadas. E como o corte é por `id` crescente, as descartadas são as regras **mais recentes** — exatamente a que você acabou de escrever e está testando. O worker emite um `warning` a cada ciclo truncado, e a tela **Detecta → Correlação** marca a linha com o selo **Não avaliada**. Para voltar a avaliá-la, desabilite regras antigas ou eleve o teto; esperar não resolve.
