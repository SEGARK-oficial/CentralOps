...
class AggregatedAlertListResponse(AlertListResponse):
    partial_errors: List[str] = Field(default_factory=list)
    is_sampled: bool = False


class ProviderOperationErrorRead(BaseModel):
    code: str
    message: str
    integration_id: Optional[int] = None
    details: Dict[str, Any] = Field(default_factory=dict)
...
class IntegrationOverviewAlertPreviewRead(BaseModel):
    alert_id: str
    title: str
    severity: str
    timestamp: Optional[str] = None
...
class IntegrationOverviewAlertPreviewListRead(BaseModel):
    items: List[IntegrationOverviewAlertPreviewRead] = Field(default_factory=list)


class IntegrationOverviewRead(BaseModel):
    integration: IntegrationRead
    health: Optional[IntegrationOverviewHealthRead] = None
    alerts_preview: Optional[IntegrationOverviewAlertPreviewListRead] = None
    alerts_preview_error: Optional[ProviderOperationErrorRead] = None
...
# ── Client (legacy, kept for backward compatibility) ──────────────────
...
class DashboardIntegrationHealthSummary(BaseModel):
    healthy: int
    degraded: int = 0
    error: int
    unknown: int
...
class DashboardIntegrationIssueSummary(BaseModel):
    integration_id: int
...
class DashboardAlertComparisonSummary(BaseModel):
    total_alerts: DashboardMetricComparison = Field(default_factory=DashboardMetricComparison)
    critical_alerts: DashboardMetricComparison = Field(default_factory=DashboardMetricComparison)


class DashboardIntegrationComparisonSummary(BaseModel):
    degraded_integrations: DashboardMetricComparison = Field(default_factory=DashboardMetricComparison)


class DashboardPrioritySummary(BaseModel):
    organization_id: Optional[int] = None
    organization_name: Optional[str] = None
    integration_id: Optional[int] = None
    integration_name: Optional[str] = None
...
class DashboardIntegrationsSummary(BaseModel):
    ...
    health: DashboardIntegrationHealthSummary
    degraded_items: List[DashboardIntegrationIssueSummary] = Field(default_factory=list)
    comparison: DashboardIntegrationComparisonSummary = Field(default_factory=DashboardIntegrationComparisonSummary)


class DashboardAlertSeveritySummary(BaseModel):
    critical: int = 0
    high: int = 0
    medium: int = 0
    low: int = 0
...
class DashboardAlertSourceSummary(BaseModel):
    integration_id: int
    integration_name: str
    organization_id: int
    organization_name: Optional[str] = None
    total: int = 0
    by_severity: DashboardAlertSeveritySummary = Field(default_factory=DashboardAlertSeveritySummary)


class DashboardAlertBucketSummary(BaseModel):
    key: str
...
class DashboardAlertSummary(BaseModel):
    total: int = 0
    by_severity: DashboardAlertSeveritySummary = Field(default_factory=DashboardAlertSeveritySummary)
    trend: List[DashboardAlertTrendPoint] = Field(default_factory=list)
    sources: List[DashboardAlertSourceSummary] = Field(default_factory=list)
    top_hosts: List[DashboardAlertBucketSummary] = Field(default_factory=list)
    top_rules: List[DashboardAlertBucketSummary] = Field(default_factory=list)
    top_mitre_ids: List[DashboardAlertBucketSummary] = Field(default_factory=list)
    top_agent_groups: List[DashboardAlertBucketSummary] = Field(default_factory=list)
    partial_errors: List[str] = Field(default_factory=list)
    latest_timestamp: Optional[str] = None
    last_query_at: Optional[str] = None
    unsupported_sources: int = 0
    window_days: int = 7
    applied_organization_id: Optional[int] = None
    applied_integration_id: Optional[int] = None
    applied_platform: Optional[str] = None
    comparison: DashboardAlertComparisonSummary = Field(default_factory=DashboardAlertComparisonSummary)
    most_critical_client: Optional[DashboardPrioritySummary] = None
    most_critical_integration: Optional[DashboardPrioritySummary] = None


class DashboardSummaryRead(BaseModel):
    organizations: DashboardOrganizationsSummary
